...
mean <- ggplot(ani, aes(meanres, abundance, shape=Group))+
  ...
  stat_cor(aes(meanres,abundance,label = paste(..p.label..), group=1), label.y = 280, label.x = 48)+
  stat_regline_equation(aes(meanres,abundance, group=1), label.y = 300, label.x = 48)+
  theme_classic() +
...
sd <- ggplot(ani, aes(sdres, abundance, shape=Group))+
  geom_point(aes(group = Group), size = 2)+
  geom_smooth(aes(group=1),method="lm",se=FALSE,col="black")+
  stat_cor(aes(sdres,abundance,label = paste(..p.label..), group=1), label.y = 280, label.x = 13)+
  stat_regline_equation(aes(sdres,abundance, group=1), label.y = 300, label.x = 13)+
  theme_classic() +
...
levels(meltponds$variable) <- c("Mean Distance (km)", "Standard Deviation Distance (km)","Mean Resistance", "Standard Deviation Resistance")
